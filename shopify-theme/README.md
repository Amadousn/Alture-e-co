...
## Homepage v8 (hero restored, benefits back to two columns)

- `sections/au-hero-video.liquid` — geometry restored to the original design (desktop min 540 px, capped at `min(80vh, 800px)`; mobile min 480 px) and the frame now follows the video ratio (`aspect-ratio: 16/9` desktop, `9/16` mobile when a mobile video or poster is set, capped at `min(84svh, 720px)`) so the whole frame is visible instead of a zoomed crop. Autoplay, muted, loop, playsinline, poster, copy and the "See the harness" CTA unchanged.
- `sections/au-benefits-tabs.liquid` — two columns again: left = eyebrow, heading, compact petrol navigation bar (four light round icons with short sentence-case labels, active item = white circle with a ring and a lighter tile, hover, focus ring, keyboard arrows), active benefit title + text, filled CTA "Choose your harness"; right = ONE large 4:5 photo that cross-fades when a benefit is selected. Mobile: eyebrow → heading → scrollable bar (two items plus part of the third, scroll-snap) → photo → title → text → full-width CTA. Only the first photo is loaded up front; the other three carry `data-src`/`data-srcset` and are attached when selected or when their tab is hovered. The 2 × 2 grid from v7 was removed.
- Customer videos stay in their own section, now headed "See the harness in action".
