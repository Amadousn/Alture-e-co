...
async function computeSignature(secret: string): Promise<string> {
    ...
    const sig = await crypto.subtle.sign("HMAC", key, enc.encode("education-session"));
...
export async function POST(req: NextRequest) {
    ...
    const validUsername = process.env.EDUCATION_USERNAME;
    const validPassword = process.env.EDUCATION_PASSWORD;
    const secret = process.env.EDUCATION_SESSION_SECRET;
    ...
    res.cookies.set("education_session", token, {
        httpOnly: true,
        sameSite: "lax",
